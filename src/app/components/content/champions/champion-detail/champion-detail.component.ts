...
export class ChampionDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.championService
      .getChampionDetail(this.championName)
      .then((response) => {
        if (response.data) {
          this.champion = response.data.champion;

          this.champion.skins.forEach((s) =>
            this.skinUrls.push(this.getSkinImageUrl(s.num))
          );
  ...
  getPassiveIconUrl(): string {
    return (
      ...
      Constants.DDRAGON_PASSIVE_ICON_URL +
      "/" +
      this.champion.passive.image.full
    );
  }
  ...
  getSpellIconURL(index: number): string {
    return (
      Constants.DDRAGON_BASE_URL +
      Constants.DDRAGON_VERSION +
      Constants.DDRAGON_SPELL_ICON_URL +
      "/" +
      this.champion.spells[index].image.full
    );
  }
  ...
  getPassiveTooltip(): string {
    return this.addTooltipDisclaimer(
      this.replaceSpecialChars(this.champion.passive.description)
    );
  }

  getSpellTooltip(index: number): string {
    return this.addTooltipDisclaimer(
      this.replaceSpecialChars(this.champion.spells[index].tooltip)
    );
  }
  ...
  getSkinImageUrl(skinNumber: number): string {
    return (
      Constants.DDRAGON_BASE_URL +
      Constants.DDRAGON_CHAMP_LOADING_URL +
      "/" +
      this.champion.name +
      "_" +
      skinNumber +
